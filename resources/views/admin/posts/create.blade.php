			...
			<form role='form' method="post" enctype="multipart/form-data">
				...
				@foreach(LaravelLocalization::getSupportedLocales() as $key => $value)
					<div class="form-group">
						...
						<input type="text" name="title[{{$key}}]" required class="form-control" value="{{ old('title[$key]') }}">
					</div>
					<div class="form-group">
						<label for="description">{{$value['name']}} Description:</label>
						<input type="text" name="description[{{$key}}]" required class="form-control" value="{{ old('description'.$key) }}">
					</div>
					<hr>
